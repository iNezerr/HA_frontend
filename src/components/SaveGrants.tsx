...
const SaveGrants: React.FC = () => {
  ...
  const fetchSavedGrants = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch saved grants';
  ...
  const handleUnsave = async (grantId: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to remove grant';
  ...
  const handleApply = async (grantId: string) => {
    try {
      await applyToGrant(grantId);
      
      setGrants(prevGrants =>
        prevGrants.map(grant =>
          grant.id === grantId ? { ...grant, is_applied: true } : grant
        )
      );
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to apply to grant';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Saved Grants</h1>
              <p className="text-gray-600">
                {totalCount} grant{totalCount !== 1 ? 's' : ''} saved for later
              </p>
            </div>
        ...
        {!loading && grants.length === 0 && !error && (
          <div className="bg-white border border-gray-200 rounded-lg p-12 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Start saving grants you're interested in to view them here later
            </p>
            <a
              href="/dashboard"
        ...
        {/* Grants List - Using GrantCard component */}
        {grants.length > 0 && (
          <div className="space-y-4">
            {grants.map((grant) => (
              <div key={grant.id} className="relative">
                <GrantCard
                  grant={grant}
                  onSave={undefined} // Don't show save button on saved grants page
                  onApply={handleApply}
                />
