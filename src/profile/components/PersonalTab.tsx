...
const PersonalTab: React.FC<PersonalTabProps> = ({
  ...
  // Clear validation errors when component receives new data - fix infinite loop
  useEffect(() => {
    if (validationErrors && validationErrors.length === 0 && Object.keys(fieldErrors).length > 0) {
      setFieldErrors({});
    }
  }, [validationErrors.length, Object.keys(fieldErrors).length]); // Use length instead of the array itself
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-gray-500">
          {personalInfo.goal?.length || 0}/1000 characters
        </p>
      </div>
    </div>
  );
