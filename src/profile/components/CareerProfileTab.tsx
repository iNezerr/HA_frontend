...
interface CareerProfileTabProps {
  ...
  setCareerProfile: (profile: CareerProfileData) => void;
  cvFile?: {
    filename?: string;
    uploadedAt?: string;
    hasCvInGcs?: boolean;
...
const CareerProfileTab = forwardRef<{ save: () => Promise<void> }, CareerProfileTabProps>(
  ({ careerProfile, setCareerProfile, cvFile, onCvUpload, validationErrors = [] }, ref) => {
    const [uploading, setUploading] = useState(false);
    const [localProfile, setLocalProfile] = useState(careerProfile);

    const handleSave = async () => {
      try {
        await setCareerProfile(localProfile);
      } catch (error) {
...
CareerProfileTab.displayName = 'CareerProfileTab';
